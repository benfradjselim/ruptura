// Package node provides the Kubernetes Node infra collector.
// It watches cluster-scoped Node objects and emits GroupControlPlane signals.
package node
...
// Collector watches Kubernetes Node objects and emits GroupControlPlane signals.
// Each node maps to one InfraSignal; Value=0 is fully healthy.
...
func (c *Collector) Name() string { return "node" }

// Probe performs a single LIST with limit=1 to verify the Nodes API is reachable.
...
// Signals returns a concurrent-safe snapshot of all current node signals.
...
func (c *Collector) listAll(ctx context.Context) (string, error) {
	...
	if err := json.NewDecoder(resp.Body).Decode(&lr); err != nil {
		resp.Body.Close()
		return "", fmt.Errorf("node: list decode: %w", err)
	}
	resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("node: list: status %d", resp.StatusCode)
	}
...
func (c *Collector) watchStream(ctx context.Context, rv string) (relistNeeded bool, err error) {
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return false, fmt.Errorf("node: watch: %w", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		return false, fmt.Errorf("node: watch: status %d", resp.StatusCode)
	}
	...
	if err := scanner.Err(); err != nil && ctx.Err() == nil {
		return false, fmt.Errorf("node: watch stream: %w", err)
	}
	return false, nil
}

func (c *Collector) upsert(item *nodeItem) {
	obj := infra.ObjectID{
...
// nodeSignal computes a single InfraSignal from a Node's condition set.
// Value = max severity across conditions; 0 = fully healthy.
func nodeSignal(obj infra.ObjectID, item *nodeItem) infra.InfraSignal {
	...
	if cond["Ready"] != "True" {
		val, severity, msg = 1.0, infra.SeverityCritical, "node not ready"
	}
	if cond["DiskPressure"] == "True" && 0.7 > val {
		val, severity, msg = 0.7, infra.SeverityWarning, "disk pressure"
	}
	if cond["MemoryPressure"] == "True" && 0.7 > val {
		val, severity, msg = 0.7, infra.SeverityWarning, "memory pressure"
	}
	if cond["PIDPressure"] == "True" && 0.5 > val {
		val, severity, msg = 0.5, infra.SeverityElevated, "PID pressure"
	}
